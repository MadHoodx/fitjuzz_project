...
export default function ProteinScreen() {
  ...
  const [allProteinFoods, setAllProteinFoods] = useState([]);
  const [loadingInitialData, setLoadingInitialData] = useState(true);
  

  // fetch all protein foods
  useEffect(() => {
    fetchAllProteinFoods();
  }, []);

  // fetch all protein foods
  const fetchAllProteinFoods = async () => {
    setLoadingInitialData(true);
    try {
      const response = await axios.get(`${EXPO_PUBLIC_ENDPOINT_API}/api/user/foodsdirect/protein`);
      if (response.data && response.data.foods) {
        setAllProteinFoods(response.data.foods);
      }
    } catch (err) {
      console.error('Error fetching all protein foods:', err);
      Alert.alert(
        'ขออภัย',
        'ไม่สามารถโหลดข้อมูลอาหารประเภทโปรตีนได้ โปรดลองใหม่อีกครั้ง',
        [{ text: 'ตกลง' }]
      );
    } finally {
  ...
  const searchFoodsLocally = (query) => {
    ...
    try {
      ...
      const results = allProteinFoods.filter(food => {
        ...
        // filter only foods with minimum 15g of protein
        const hasSufficientProtein = food.nutritionPer100g?.protein >= 15;
        
        return (nameMatch || descMatch || tagMatch) && hasSufficientProtein;
      });
    ...
    >
      ...
      <View style={styles.contentContainer}>
        {searching ? (
          ...
          <FoodCategoryList
            categoryName="โปรตีน"
            apiPath="/api/user/foodsdirect/protein"
            macroColor="#1976D2"
            emptyMessage="ไม่พบข้อมูลอาหารประเภทโปรตีน"
            errorMessage="ไม่สามารถโหลดข้อมูลอาหารประเภทโปรตีนได้"
            navigation={navigation}
            disableSearch={true}
            sortType={sortType}
